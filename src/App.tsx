...
export default function App() {
  ...
  // --- Randomize Questions (Instant Local Randomization) ---
  const fetchQuestions = () => {
    // We already have QUESTIONS imported, so we can randomize locally for instant start
    ...
    // Optionally sync with API in background if needed, but don't block
    fetch('/api/questions')
      .then(res => res.json())
      .then(data => {
        if (data && data.length >= 16) {
          // If we haven't started the game yet, we can update to server-side questions
          if (gameState === 'START') {
            setCurrentQuestions(data);
          }
        }
      })
      .catch(e => console.error('Background question fetch failed', e));
  };
  ...
  const deleteEntry = async (name: string) => {
    try {
      const res = await fetch(`/api/leaderboard/${encodeURIComponent(name)}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  const handleNameClick = (name: string) => {
    const newCount = (clickTracker[name] || 0) + 1;
    if (newCount >= 5) {
      deleteEntry(name);
      setClickTracker({ ...clickTracker, [name]: 0 });
    } else {
      setClickTracker({ ...clickTracker, [name]: newCount });
      // Reset after 2 seconds of inactivity
      setTimeout(() => {
        setClickTracker(prev => ({ ...prev, [name]: 0 }));
      }, 2000);
    }
  };
  ...
  const submitScore = async (score: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast('تم حفظ النتيجة بنجاح في لائحة الأوائل!', 'success');
        // Refresh leaderboard data if modal is open
        if (showLeaderboard) fetchLeaderboard();
        return true;
      } else {
        const err = await response.json();
        console.error('Server error saving score:', err);
        showToast('فشل حفظ النتيجة. حاول مرة أخرى.', 'error');
        return false;
      }
    } catch (e) {
      console.error('Network error saving score:', e);
  ...
  const playResultSound = (url: string) => {
    const audio = new Audio(url);
    audio.play().catch(e => console.log('Audio play blocked', e));
    // We don't assign this to audioRef.current so it won't be paused by playSound()
  };
  ...
  const checkAnswer = async () => {
    ...
    if (isCorrect) {
      ...
      if (currentLevel === 15) {
        // Show win screen sooner, save in background
        setTimeout(() => {
          setGameState('WIN');
      ...
      setTimeout(() => {
        setGameState('LOSE');
        submitScore(finalPoints);
      }, 4000); // Increased timeout to let the 'Bad' sound play
    }
  };
  ...
  const useAiHint = () => {
    ...
    // Use the hint from the question file
  ...
  const takeScreenshot = async (label: string) => {
    ...
    try {
      const canvas = await html2canvas(gameRef.current, {
        ...
        allowTaint: true,
        onclone: (clonedDoc) => {
          // Remove elements that might cause issues or aren't needed in screenshot
          const elements = clonedDoc.getElementsByTagName('*');
          for (let i = 0; i < elements.length; i++) {
            const el = elements[i] as HTMLElement;
            if (el.style && el.style.color && (el.style.color.includes('oklch') || el.style.color.includes('oklab'))) {
              el.style.color = '#d4af37'; // Fallback to gold
            }
            if (el.style && el.style.backgroundColor && (el.style.backgroundColor.includes('oklch') || el.style.backgroundColor.includes('oklab'))) {
              el.style.backgroundColor = '#1a4d4d'; // Fallback to teal
            }
          }
        }
      });
      console.log(`Screenshot saved for ${playerName}: ${label}`);
    } catch (e) {
  ...
  const downloadCertificate = async () => {
    ...
    try {
      const canvas = await html2canvas(certElement, {
        ...
        logging: false,
        onclone: (clonedDoc) => {
          // Fix for oklab/oklch in html2canvas
          ...
          style.innerHTML = `
            * { 
            ...
            .certificate-gradient {
              background: linear-gradient(to bottom left, rgba(26, 77, 77, 1), rgba(26, 77, 77, 0.8), transparent) !important;
            }
          `;
      ...
      alert('عذراً، حدث خطأ أثناء حفظ الشهادة. يرجى المحاولة مرة أخرى.');
    }
  };
            ...
            <div className="text-center space-y-4 md:space-y-6 flex flex-col h-full">
              ...
              <div className="flex gap-2 p-1 bg-black/20 rounded-xl border border-ramadan-gold/20">
                <button 
                  ...
                  className={`flex-1 py-2 rounded-lg font-bold transition-all text-sm ${leaderboardType === 'all' ? 'bg-ramadan-gold text-ramadan-teal' : 'text-ramadan-cream hover:bg-white/5'}`}
                >
                  الكل
                </button>
                <button 
                  ...
                  className={`flex-1 py-2 rounded-lg font-bold transition-all text-sm ${leaderboardType === 'week' ? 'bg-ramadan-gold text-ramadan-teal' : 'text-ramadan-cream hover:bg-white/5'}`}
                >
                  الأسبوع
                </button>
                <button 
                  onClick={() => {
                    setLeaderboardType('month');
                    fetchLeaderboard('month');
                  }}
                  className={`flex-1 py-2 rounded-lg font-bold transition-all text-sm ${leaderboardType === 'month' ? 'bg-ramadan-gold text-ramadan-teal' : 'text-ramadan-cream hover:bg-white/5'}`}
                >
                  الشهر
                </button>
              </div>
                  ...
                  leaderboardData.map((entry, idx) => (
                    <div key={idx} className="flex items-center justify-between bg-black/20 p-3 md:p-4 rounded-2xl border border-ramadan-gold/20">
                      <div className="flex items-center gap-3 md:gap-4">
                        ...
                        <span 
                          className="text-white font-bold cursor-pointer select-none active:scale-95 transition-transform"
              ...
              <p className="text-[10px] text-ramadan-gold/40 text-center">
                * يتم تحديث اللائحة الأسبوعية كل يوم اثنين
              </p>
            </div>
          </motion.div>
        </motion.div>
            ...
            <button 
              onClick={async () => {
                try {
                  ...
                  fetchQuestions(); // Refresh questions after sync
                } catch (e) {
                  showToast('فشل تحديث الأسئلة', 'error');
                }
              }}
              className="w-full bg-black/20 text-ramadan-gold/60 text-xs py-2 rounded-full hover:bg-black/40 transition-all flex items-center justify-center gap-2"
            >
              <RefreshCw className="w-3 h-3" />
              تحديث الأسئلة من المصدر
            </button>
          </div>
        </motion.div>
        ...
        {showCertificate ? (
          <div className="w-full max-w-4xl">
            <div id="certificate" className="w-full bg-white p-6 sm:p-10 md:p-16 text-[#1a4d4d] rounded-sm border-b-[10px] md:border-b-[15px] border-[#d4af37] shadow-2xl relative flex flex-col justify-center min-h-[400px] md:aspect-[1.414/1]">
              {/* Top Decorative Wave */}
              <div className="absolute top-0 right-0 w-32 md:w-64 h-32 md:h-64 certificate-gradient -translate-y-1/2 translate-x-1/2 rotate-45" 
            ...
            <div className="mt-4 text-center">
              <button 
                onClick={() => submitScore(displayPoints)}
                className="text-ramadan-gold text-sm underline hover:text-ramadan-cream"
              >
                هل لم تظهر نتيجتك في اللائحة؟ اضغط هنا لحفظها يدوياً
              </button>
            </div>
          </div>
        ) : (
          ...
          >
            ...
            {displayPoints > 0 && (
              <button 
                ...
                className="text-ramadan-gold text-sm underline hover:text-ramadan-cream"
              >
                هل لم تظهر نتيجتك؟ اضغط هنا لحفظها يدوياً
              </button>
            )}
          </motion.div>
        )}
      ...
      <div className="flex flex-col md:flex-row flex-1">
        ...
        <div className="flex-1 flex flex-col relative z-10">
          ...
          <div className="p-3 md:p-6 flex flex-col md:flex-row justify-between items-center gap-2 md:gap-4">
            {/* Top Row for Mobile: Moon, Title, Sidebar Toggle */}
            ...
            <div className="flex flex-col items-center md:items-end gap-2 md:gap-4 w-full md:w-auto">
              ...
              <div className="flex gap-1.5 md:gap-2">
                <button onClick={useFiftyFifty} disabled={!lifelines.fiftyFifty || gameState !== 'PLAYING'} className={`lifeline-circle w-10 h-10 md:w-12 md:h-12 ${!lifelines.fiftyFifty && 'opacity-30'}`} title="50:50">
                  <span className="text-[10px] md:text-xs font-bold">50/50</span>
                </button>
                <button onClick={useAiHint} disabled={!lifelines.aiHint || gameState !== 'PLAYING' || isAiLoading} className={`lifeline-circle w-10 h-10 md:w-12 md:h-12 ${!lifelines.aiHint && 'opacity-30'}`} title="AI Hint">
                  {isAiLoading ? <RefreshCw className="w-4 h-4 md:w-5 md:h-5 animate-spin" /> : <span className="text-[10px] md:text-xs font-bold">AI</span>}
                </button>
                <button onClick={useCallHelp} disabled={!lifelines.call || gameState !== 'PLAYING'} className={`lifeline-circle w-10 h-10 md:w-12 md:h-12 ${!lifelines.call && 'opacity-30'}`} title="Call">
                  <Phone className="w-4 h-4 md:w-5 md:h-5" />
                </button>
                <button onClick={useSwitchQuestion} disabled={!lifelines.switch || gameState !== 'PLAYING'} className={`lifeline-circle w-10 h-10 md:w-12 md:h-12 ${!lifelines.switch && 'opacity-30'}`} title="Switch">
        ...
        {/* Right Sidebar - Progress (Drawer on Mobile) */}
        <div className={`
